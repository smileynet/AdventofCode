...
def return_hexdigest(secret_key):
    result = hashlib.md5(secret_key.encode())
    hex_digest = result.hexdigest()
    return hex_digest

secret_key_base = "ckczppom"

five_zeros = "00000"
six_zeros = "000000"

for number in range(10000000000):
    secret_key = secret_key_base + str(number)
    hex_digest = return_hexdigest(secret_key)
    if hex_digest.startswith(six_zeros):
        print(f'number: {number} hex_digest: {hex_digest}')
        break
